...
function getDefaultValues({
  ...
  if (datasourceProvider === 'cockroachdb') {
    if (wordsBeforePosition.some((a) => a.includes('sequence('))) {
      ...
      const sequenceProperties = ['virtual', 'minValue', 'maxValue', 'cache', 'increment', 'start', 'noCycle']

      // No suggestions if virtual or noCycle is present
      if (currentLine.includes('virtual') || currentLine.includes('noCycle')) {
        return suggestions
      }
