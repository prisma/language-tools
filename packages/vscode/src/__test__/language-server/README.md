...
The integration runner opens `tests/fixtures/integration-workspace.code-workspace`. Its single root is a pnpm workspace importer containing the lockfile-resolved real `prisma@8.0.0-rc.7` CLI, matching Prisma 8 engine and Postgres ORM packages, and a valid `prisma.config.ts` whose contract is only the marked `next.prisma` fixture.
