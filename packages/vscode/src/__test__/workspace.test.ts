...
suite('Multi-root integration workspace', () => {
  ...
  test('routes unsaved documents exclusively across real root-local clients', async () => {
    ...
    const secondDocumentA = await vscode.workspace.openTextDocument(getWorkspaceDocUri(rootA, 'second.prisma'))
    const documentB = await vscode.workspace.openTextDocument(getWorkspaceDocUri(rootB, 'schema.prisma'))
    const missingDocument = await vscode.workspace.openTextDocument(getWorkspaceDocUri(missingRoot, 'schema.prisma'))
    ...
    assert.deepStrictEqual(activeOwnerKeys(localAState, documentA.uri), new Set([localOwnerKey(rootA.uri.toString())]))
    assert.strictEqual(
    ...
    assert.strictEqual(missingState.localClients.startCountsByWorkspaceFolderUri[missingRoot.uri.toString()], undefined)
